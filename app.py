...
#
#  Design      : Editorial "Sovereign Obsidian" — Playfair Display + IBM Plex Mono
#                Deep charcoal backgrounds, amber-gold accents, crisp data grid
#
...
#  Palette: near-black #0C0F14, deep navy #111827, amber-gold #D4A017
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
/* ── Google Fonts ───────────────────────────────────────────────────────── */
...
/* ── App background ─────────────────────────────────────────────────────── */
.main, .stApp { background: #0C0F14 !important; }
.block-container {
    padding: 0 !important;
...
def lay(**kw):
    d = dict(
        ...
        font=dict(family=FONT_M, size=10, color="#4B5563"),
        margin=dict(l=46, r=16, t=32, b=40),
        legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(size=10, color="#6B7280"),
        ...
        yaxis=dict(gridcolor=GRID_C, zerolinecolor=GRID_C,
                   tickfont=dict(color="#4B5563", size=10)),
        hoverlabel=dict(bgcolor="#1E2530", bordercolor=GOLD,
                        font=dict(color="#F3F4F6", family=FONT_M, size=11)),
    )
    d.update(kw)
    return d
...
def page_header(eyebrow, title, subtitle):
    return f"""
<div style="background:linear-gradient(160deg,#111827 0%,#0C0F14 60%,#0F1520 100%);
            border-bottom:1px solid {BORDER};padding:36px 40px 28px;
  ...
  <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:{GOLD};
              ...
              opacity:0.7;">{eyebrow}</div>
  <div style="font-family:'Playfair Display',serif;font-size:38px;font-weight:900;
              color:#F9FAFB;line-height:1.1;margin-bottom:8px;
              letter-spacing:-0.5px;">{title}</div>
  <div style="font-family:'Outfit',sans-serif;font-size:14px;color:#4B5563;
...
def kpi_card(label, value, unit="", change="", change_type="neutral",
    ...
    chg_color = {"up": ROSE, "down": SAGE, "neutral": "#4B5563"}.get(change_type, "#4B5563")
    return f"""
<div style="background:{PANEL};border:1px solid {BORDER};border-radius:10px;
  ...
  <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#4B5563;
  ...
  <div style="font-family:'Playfair Display',serif;font-size:32px;font-weight:900;
      ...
      {value}<span style="font-size:16px;color:{accent};opacity:.7;">{unit}</span>
  </div>
  ...
  {"<div style='font-family:IBM Plex Mono,monospace;font-size:9px;color:#374151;margin-top:4px;'>"+source+"</div>" if source else ""}
  {"<div style='font-family:IBM Plex Mono,monospace;font-size:9px;color:#374151;margin-top:2px;'>"+note+"</div>" if note else ""}
</div>"""
...
<div style="margin:28px 0 16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:4px;">
    <div style="width:3px;height:28px;background:linear-gradient(180deg,{accent},{accent}55);
                border-radius:2px;flex-shrink:0;"></div>
    <div style="font-family:'Playfair Display',serif;font-size:20px;font-weight:700;
                color:#F9FAFB;">{title}</div>
  </div>
  {"<div style='font-family:IBM Plex Mono,monospace;font-size:10px;color:#4B5563;margin-left:15px;letter-spacing:1px;'>"+subtitle+"</div>" if subtitle else ""}
</div>"""

def formula_box():
    return f"""
<div style="background:#080C12;border:1px solid rgba(212,160,23,0.2);
            border-radius:10px;padding:20px 24px;margin:14px 0;
            font-family:'IBM Plex Mono',monospace;overflow-x:auto;">
  <div style="font-size:9px;color:#4B5563;letter-spacing:2px;
  ...
  <div style="font-size:13px;line-height:2.6;white-space:nowrap;">
    ...
    <span style="color:#374151;font-size:10px;"> [neutral rate] </span>
    <span style="color:#6B7280;">+ </span>
    <span style="color:{TEAL};">λ · OPR<sub>t</sub></span>
    <span style="color:#374151;font-size:10px;"> [CBSL transmission] </span>
    <span style="color:#6B7280;">+ </span>
    <span style="color:{SAGE};">β₁ · (US10Y<sub>t</sub> + σ<sub>LKA</sub>)</span>
    <span style="color:#374151;font-size:10px;"> [Mundell-Fleming] </span>
    <span style="color:#6B7280;">+ </span>
    <span style="color:{ROSE};">γ₁·D/GDP<sub>t</sub> + γ₂·FD<sub>t</sub></span>
    <span style="color:#374151;font-size:10px;"> [solvency/crowding-out] </span>
    <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <span style="color:#6B7280;">+ </span>
    <span style="color:{WARM};">ϕ · (Oil<sub>t</sub> × W)</span>
    <span style="color:#374151;font-size:10px;"> [cost-push] </span>
    <span style="color:#6B7280;">+ </span>
    <span style="color:{VIOLET};">θ · S<sub>t</sub></span>
    <span style="color:#374151;font-size:10px;"> [seasonal] </span>
    <span style="color:#6B7280;">+ </span>
    <span style="color:{GOLD};">Ω · (1/GOR<sub>t</sub>)</span>
    <span style="color:#374151;font-size:10px;"> [ext. buffer] </span>
    <span style="color:#6B7280;">+ </span>
    <span style="color:{TEAL};">δ·(π<sub>t</sub>−π*)</span>
    <span style="color:#374151;font-size:10px;"> [Fisher gap] </span>
...
def source_note(text):
    return f"""
<div style="background:rgba(212,160,23,0.04);border-left:2px solid rgba(212,160,23,0.25);
...
def forecast_card(tenor, value, ci_lo, ci_hi, actual, color):
    ...
    dir_col = ROSE if diff > 0.05 else (SAGE if diff < -0.05 else "#6B7280")
    return f"""
<div style="background:#080C12;border:1px solid {BORDER};border-radius:12px;
  ...
  <div style="position:absolute;bottom:-20px;right:-20px;width:80px;height:80px;
              border-radius:50%;background:radial-gradient(circle,{color}11,transparent);"></div>
  <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#374151;
              letter-spacing:2.5px;text-transform:uppercase;margin-bottom:12px;">{tenor}</div>
  <div style="font-family:'Playfair Display',serif;font-size:44px;font-weight:900;
  ...
  <div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#374151;
              margin-top:10px;">95% CI: {ci_lo:.2f}% – {ci_hi:.2f}%</div>
...
def adj_panel_header(title, icon, color=GOLD):
    return f"""
<div style="background:linear-gradient(135deg,#111827,#0C0F14);
  ...
  <div>
    <div style="font-family:'Playfair Display',serif;font-size:14px;
                font-weight:700;color:#F9FAFB;">{title}</div>
  </div>
...
with st.sidebar:
    st.markdown(f"""
    <div style="background:linear-gradient(180deg,#111827,#0A0D12);
                border-bottom:1px solid {BORDER};padding:24px 20px 20px;">
      ...
      <div style="font-family:'IBM Plex Mono',monospace;font-size:10px;
                  ...
                  border-top:1px solid {BORDER};">📅 {TODAY_STR}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="padding:16px 20px;border-top:1px solid {BORDER};margin-top:8px;">
      <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#374151;
      ...
      <div style="font-family:'IBM Plex Mono',monospace;font-size:12px;line-height:2.2;">
        ...
        <span style="color:#9CA3AF;">→ {CUR['y91']:.2f}%</span><br>
        <span style="color:{TEAL};">182D </span>
        <span style="color:#9CA3AF;">→ {CUR['y182']:.2f}%</span><br>
        <span style="color:{VIOLET};">364D </span>
        <span style="color:#9CA3AF;">→ {CUR['y364']:.2f}%</span><br>
        <span style="color:#6B7280;">OPR  </span>
        <span style="color:#9CA3AF;">→ {CUR['opr']:.2f}%</span><br>
        <span style="color:{SAGE};">GOR  </span>
        <span style="color:#9CA3AF;">→ ${CUR['gor']:.3f}Bn</span>
      </div>
    </div>
    <div style="padding:12px 20px;border-top:1px solid {BORDER};">
      <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#374151;
...
if page == "Dashboard":
    ...
    if CUR["brent"] > 78:
        ...
        <div style="background:rgba(251,113,133,0.07);border:1px solid rgba(251,113,133,0.2);
          ...
          <span style="font-family:'IBM Plex Mono',monospace;font-size:11px;color:#FDA4AF;">
    ...
    pairs = [
        ...
        (k4,"OPR POLICY RATE",f"{CUR['opr']:.2f}","%","Unchanged Jan 27, 2026","neutral","CBSL MPR No.1/2026","#9CA3AF"),
    ...
    with ch2:
        ...
        for row in [
            ...
            ("AWCMR", f"{CUR['awcmr']:.2f}%", "#9CA3AF"),
            ("AWPR", f"{CUR['awpr']:.2f}%", "#9CA3AF"),
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;
              ...
              <span style="font-family:'IBM Plex Mono',monospace;font-size:10px;
                           color:#4B5563;">{row[0]}</span>
              <span style="font-family:'IBM Plex Mono',monospace;font-size:11px;
                           color:{row[2]};font-weight:500;">{row[1]}</span>
            </div>""", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with pb:
        ...
        st.markdown(f"""
        <div style="background:#080C12;border-radius:6px;padding:10px 12px;margin-top:8px;">
          <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#374151;
    ...
    with pd_:
        ...
        st.markdown(f"""
        <div style="background:#080C12;border-radius:6px;padding:10px 12px;margin-top:8px;">
          <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#374151;margin-bottom:4px;">
          ...
          <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#4B5563;margin-top:6px;">
    ...
    with pg:
        ...
        <div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#4B5563;
                    margin-top:8px;padding:8px;background:#080C12;border-radius:6px;">
          Auto-detected: <span style="color:{VIOLET};">{auto_q}</span> &nbsp;·&nbsp;
          Contribution: <span style="color:{VIOLET};">{theta*seas_v:+.3f}%</span>
        </div>""", unsafe_allow_html=True)
    ...
    with ph:
        ...
        st.markdown(f"""
        <div style="background:#080C12;border-radius:6px;padding:10px 12px;margin-top:4px;">
          <div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#4B5563;margin-bottom:6px;">
              Current spread implied by offsets</div>
          <div style="display:flex;gap:16px;">
            <div style="font-family:'IBM Plex Mono',monospace;font-size:11px;color:{GOLD};">
                91D → {off91:+.2f}%</div>
            <div style="font-family:'IBM Plex Mono',monospace;font-size:11px;color:{TEAL};">
                182D → {off182:+.2f}%</div>
            <div style="font-family:'IBM Plex Mono',monospace;font-size:11px;color:{VIOLET};">
                364D → {off364:+.2f}%</div>
          </div>
        </div>""", unsafe_allow_html=True)
    ...
    with dc1:
        ...
        html_d = f"""
        <div style="background:{PANEL};border:1px solid {BORDER};border-radius:10px;overflow:hidden;">
          <table style="width:100%;border-collapse:collapse;">
            <thead>
              <tr style="background:#080C12;border-bottom:1px solid {BORDER};">
                <th style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#374151;
            ...
            html_d += f"""
              <tr style="border-bottom:1px solid rgba(255,255,255,0.03);">
                <td style="font-family:'IBM Plex Mono',monospace;font-size:11px;
                           color:#9CA3AF;padding:9px 14px;">{term}</td>
                <td style="font-family:'IBM Plex Mono',monospace;font-size:12px;
                           color:{vc};padding:9px 14px;text-align:right;">{val:+.4f}%</td>
                <td style="padding:9px 14px;text-align:right;">
                  <div style="display:flex;align-items:center;justify-content:flex-end;gap:8px;">
                    <div style="width:50px;height:3px;background:rgba(255,255,255,0.05);border-radius:2px;">
                      <div style="width:{min(pct/1.2,100):.0f}%;height:100%;background:{vc};border-radius:2px;"></div>
                    </div>
                    <span style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#4B5563;
        ...
        html_d += f"""
              <tr style="background:#080C12;border-top:1px solid {BORDER};">
                <td style="font-family:'IBM Plex Mono',monospace;font-size:11px;color:{GOLD};
                           padding:10px 14px;font-weight:600;">Base Sum (91D)</td>
                <td style="font-family:'IBM Plex Mono',monospace;font-size:14px;color:{GOLD};
                           padding:10px 14px;text-align:right;font-weight:600;">{base_sum:.3f}%</td>
                <td></td>
              </tr>
            </tbody></table></div>"""
    ...
    with dc2:
        ...
        for lbl_, val_, col_, theory_ in sens:
            ...
            <div style="display:flex;align-items:center;gap:10px;
              ...
              <div style="font-family:'IBM Plex Mono',monospace;font-size:10px;
                          color:#6B7280;width:130px;flex-shrink:0;">{lbl_}</div>
              ...
              <div style="font-family:'IBM Plex Mono',monospace;font-size:11px;
                          color:{col_};width:60px;text-align:right;">{val_:+.3f}pp</div>
            </div>
            <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;
                        color:#374151;margin-left:140px;margin-bottom:4px;">{theory_}</div>
            """, unsafe_allow_html=True)
    ...
    fig_fit.add_annotation(
        ...
        showarrow=False,
        bgcolor="rgba(8,12,18,0.85)", bordercolor=GOLD, borderwidth=1,
        font=dict(color="#6B7280", size=10, family=FONT_M))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div style="background:rgba(14,165,233,0.06);border:1px solid rgba(14,165,233,0.15);
      ...
      <span style="font-family:'IBM Plex Mono',monospace;font-size:11px;color:#7DD3FC;">
    ...
    st.markdown(f"""
    <div style="background:{PANEL};border:1px solid rgba(212,160,23,0.25);
                border-radius:10px;padding:20px 24px;margin-top:12px;
                display:flex;gap:40px;align-items:center;flex-wrap:wrap;">
      <div>
        <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;
                    color:#374151;letter-spacing:2px;margin-bottom:6px;">CUSTOM 91D</div>
        <div style="font-family:'Playfair Display',serif;font-size:36px;
                    font-weight:900;color:{GOLD};">{cf91:.2f}%</div>
      </div>
      <div>
        <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;
                    color:#374151;letter-spacing:2px;margin-bottom:6px;">CUSTOM 182D</div>
        <div style="font-family:'Playfair Display',serif;font-size:36px;
                    font-weight:900;color:{TEAL};">{cf182:.2f}%</div>
      </div>
      <div>
        <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;
                    color:#374151;letter-spacing:2px;margin-bottom:6px;">CUSTOM 364D</div>
        <div style="font-family:'Playfair Display',serif;font-size:36px;
                    font-weight:900;color:{VIOLET};">{cf364:.2f}%</div>
      </div>
    </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
          ...
          <div style="padding-left:16px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
              ...
              <span style="font-family:'Playfair Display',serif;font-size:16px;
                           font-weight:700;color:#F9FAFB;">{name}</span>
            </div>
            <div style="font-family:'Outfit',sans-serif;font-size:13px;
                        color:#6B7280;line-height:1.8;">{content}</div>
          </div>
        </div>"""
    ...
    st.markdown(f"""
    <div style="background:#080C12;border:1px solid rgba(251,113,133,0.15);
                border-radius:10px;padding:20px 24px;">
      <div style="font-family:'Outfit',sans-serif;font-size:13px;color:#6B7280;
                  line-height:2.0;">
        <span style="color:{ROSE};font-weight:600;">①</span> <b style="color:#9CA3AF;">Sample size:</b>
        ...
        <span style="color:{ROSE};font-weight:600;">②</span> <b style="color:#9CA3AF;">Structural breaks:</b>
        ...
        <span style="color:{ROSE};font-weight:600;">③</span> <b style="color:#9CA3AF;">Backward-looking inflation:</b>
        ...
        <span style="color:{ROSE};font-weight:600;">④</span> <b style="color:#9CA3AF;">Reduced form:</b>
        ...
        <span style="color:{ROSE};font-weight:600;">⑤</span> <b style="color:#9CA3AF;">Disclaimer:</b>
        Forecasts are indicative only. <b style="color:{ROSE};">This platform is NOT financial advice.</b>
...
# ══════════════════════════════════════════════════════════════════════════════
st.markdown(f"""
<div style="background:#080C12;border-top:1px solid {BORDER};
