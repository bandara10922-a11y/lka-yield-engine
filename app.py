...
#
#  Design      : "Sovereign Obsidian" — Arial system font · Dark · Animated
...
# ══════════════════════════════════════════════════════════════════════════════
#  DESIGN SYSTEM  —  "Sovereign Obsidian"
#  Font  : Arial (system) everywhere — clean, professional, universally readable
#  Palette: near-black #0A0D12, deep navy #111827, amber-gold #D4A017
#  Animations: fade-up, counter rolls, shimmer, pulse dots, bar grow,
#              hover lifts, slide-in panels, glowing borders, chart glow,
#              staggered entrance, scroll-triggered reveals, text glow
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
/* ── Base — Arial everywhere, bright text ──────────────────────────────────── */
...
/* ── App background ──────────────────────────────────────────────────────── */
.main, .stApp { background: #0A0D12 !important; }
.block-container { padding: 0 !important; max-width: 100% !important; }

/* ════════════════════════════════════════════════════
   KEYFRAME ANIMATIONS  —  Rich & Varied
════════════════════════════════════════════════════ */
@keyframes fadeUp {
...
@keyframes pulse {
    ...
    50%      { opacity:.7; box-shadow:0 0 0 10px rgba(212,160,23,0); }
}
@keyframes pulseDot {
...
@keyframes borderGlow {
    0%,100% { box-shadow:0 0 0 1px rgba(212,160,23,0.15),0 4px 16px rgba(0,0,0,0.3); }
    50%      { box-shadow:0 0 18px 2px rgba(212,160,23,0.28),0 0 0 1px rgba(212,160,23,0.4),0 8px 32px rgba(0,0,0,0.5); }
}
@keyframes countUp {
...
@keyframes goldPulse {
    ...
    50%      { text-shadow:0 0 30px rgba(212,160,23,0.8),0 0 60px rgba(212,160,23,0.3); }
}

/* ── Animation utility classes ────────────────────────────────────────────── */
.anim-fade-up    { animation: fadeUp        .6s cubic-bezier(.22,.68,0,1.15) both; }
.anim-fade-in    { animation: fadeIn        .5s ease both; }
.anim-slide-r    { animation: slideRight    .55s cubic-bezier(.22,.68,0,1.15) both; }
.anim-slide-l    { animation: slideLeft     .55s cubic-bezier(.22,.68,0,1.15) both; }
.anim-slide-d    { animation: slideDown     .45s cubic-bezier(.22,.68,0,1.15) both; }
.anim-scale      { animation: scaleIn       .5s cubic-bezier(.22,.68,0,1.15) both; }
.anim-scale-b    { animation: scaleInBounce .6s cubic-bezier(.22,.68,0,1.15) both; }
.anim-count      { animation: countUp       .65s cubic-bezier(.22,.68,0,1.15) both; }
...
/* ── Scan line overlay (subtle cinematic feel on headers) ──────────────────── */
.scan-overlay::after {
    content:'';
...
section[data-testid="stSidebar"] {
    background: linear-gradient(180deg,#0D1421 0%,#0A0D12 100%) !important;
    border-right: 1px solid rgba(212,160,23,0.2) !important;
    padding-top: 0 !important;
    animation: slideRight .55s cubic-bezier(.22,.68,0,1.15) both;
}
...
div[data-testid="stNumberInput"] input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(212,160,23,0.2),
                0 0 16px rgba(212,160,23,0.15) !important;
}
...
div[data-testid="stTabs"] button[role="tab"] {
    ...
    letter-spacing: 1.5px; border-radius: 0 !important;
...
div[data-testid="stTabs"] button[role="tab"][aria-selected="true"] {
    ...
    text-shadow: 0 0 14px rgba(212,160,23,0.5) !important;
}

/* Buttons */
.stButton > button {
    background: linear-gradient(135deg,#8B6000,#C4880A,#F0C040) !important;
    color: #0A0D12 !important; border: none !important;
    border-radius: 8px !important;
    font-family: Arial, Helvetica, sans-serif !important;
    font-weight: 900 !important; font-size: 13px !important;
    letter-spacing: .8px !important; padding: 12px 28px !important;
...
.stButton > button:hover {
    ...
    box-shadow: 0 12px 36px rgba(212,160,23,0.5),
                0 0 0 1px rgba(212,160,23,0.45) !important;
}
...
.stDownloadButton > button {
    ...
    color: #D4A017 !important;
    border: 1px solid rgba(212,160,23,0.38) !important;
    border-radius: 8px !important;
    font-family: Arial, Helvetica, sans-serif !important;
...
.stDownloadButton > button:hover {
    ...
    box-shadow: 0 0 18px rgba(212,160,23,0.3) !important;
}
...
/* Caption */
.stCaption { color: #64748B !important; font-family: Arial, sans-serif !important; font-size:11px !important; }

/* Radio */
div[data-testid="stRadio"] label { color: #CBD5E1 !important; font-family: Arial, sans-serif !important; font-weight:700 !important; }

/* Dropdown */
div[data-baseweb="popover"] { background: #131B27 !important; border-color: rgba(212,160,23,0.28) !important; }
div[data-baseweb="menu"] li { color: #E2E8F0 !important; background: #131B27 !important; font-family: Arial, sans-serif !important; font-weight:600 !important; }
div[data-baseweb="menu"] li:hover { background: rgba(212,160,23,0.14) !important; }

/* Multiselect */
div[data-baseweb="tag"] { background: rgba(212,160,23,0.2) !important; border:1px solid rgba(212,160,23,0.38) !important; }
div[data-baseweb="tag"] span { color: #D4A017 !important; font-family: Arial, sans-serif !important; font-weight:700 !important; }

/* Markdown text — ensure bright */
...
div[data-testid="stMetric"] {
    background:#111827; border:1px solid rgba(212,160,23,0.18);
    border-radius:10px; padding:12px;
    animation: fadeUp .5s both;
}
div[data-testid="stMetricLabel"] p { color: #94A3B8 !important; font-family: Arial, sans-serif !important; font-size:11px !important; font-weight:700 !important; letter-spacing:1.2px; }
div[data-testid="stMetricValue"] { color: #D4A017 !important; font-family: Arial, sans-serif !important; font-size:28px !important; font-weight:900 !important; }
...
::-webkit-scrollbar-thumb { background:rgba(212,160,23,0.32); border-radius:3px; }
::-webkit-scrollbar-thumb:hover { background:rgba(212,160,23,0.55); }
...
.kpi-hover:hover {
    ...
    box-shadow: 0 16px 40px rgba(0,0,0,0.5),
                ...
                0 0 24px rgba(212,160,23,0.12) !important;
}

/* ── Chart container hover glow (applied via JS) ──────────────────────────── */
.chart-glow { transition: box-shadow .35s ease !important; }
</style>
...
(function() {
  function runAll() {
    ...
    document.querySelectorAll('.anim-fade-up,.anim-scale,.anim-slide-r').forEach(el => {
      if (!el.dataset.observed) { el.dataset.observed = '1'; io.observe(el); }
    });

    /* Number counter roll */
    document.querySelectorAll('.roll-num').forEach(el => {
    ...
    document.querySelectorAll('[data-testid="stPlotlyChart"] .js-plotly-plot').forEach(el => {
      ...
      el.addEventListener('mouseenter', () => {
        el.style.boxShadow = '0 0 28px rgba(212,160,23,0.2),0 0 0 1px rgba(212,160,23,0.15)';
    ...
    /* Stagger KPI cards */
    document.querySelectorAll('.kpi-hover').forEach((el, i) => {
      if (!el.dataset.stag) { el.dataset.stag = '1'; el.style.animationDelay = (i * 0.08) + 's'; }
    });

    /* Stagger forecast cards */
    document.querySelectorAll('.anim-scale').forEach((el, i) => {
      if (!el.dataset.stag) { el.dataset.stag = '1'; el.style.animationDelay = (i * 0.12) + 's'; }
    });
  }
...
FONT_M  = "Arial, Helvetica, sans-serif"
FONT_B  = "Arial, Helvetica, sans-serif"

def lay(**kw):
    d = dict(
        paper_bgcolor=PANEL, plot_bgcolor="#0D1420",
        font=dict(family=FONT_M, size=11, color="#94A3B8"),
        margin=dict(l=48, r=18, t=34, b=42),
        legend=dict(
            bgcolor="rgba(17,24,39,0.9)",
            bordercolor="rgba(212,160,23,0.2)", borderwidth=1,
        ...
        hoverlabel=dict(bgcolor="#1E293B", bordercolor=GOLD,
                        font=dict(color="#F1F5F9", family=FONT_M, size=12)),
    )
    d.update(kw)
    return d
...
def page_header(eyebrow, title, subtitle):
    return f"""
<div class="anim-fade-in" style="background:linear-gradient(160deg,#0D1421 0%,#111827 55%,#0A0D12 100%);
            border-bottom:2px solid rgba(212,160,23,0.3);padding:38px 40px 30px;
  ...
  <div style="position:absolute;top:-80px;right:-80px;width:360px;height:360px;
              ...
              pointer-events:none;animation:floatUp 5s ease-in-out infinite;"></div>
  <div style="position:absolute;bottom:-60px;left:15%;width:420px;height:220px;
              background:radial-gradient(ellipse,rgba(34,211,238,0.04) 0%,transparent 70%);
              pointer-events:none;"></div>
  <div style="position:absolute;top:0;left:0;right:0;height:2px;
              background:linear-gradient(90deg,transparent,rgba(212,160,23,0.5),rgba(212,160,23,0.8),rgba(212,160,23,0.5),transparent);
              animation:shimmer 3s infinite;background-size:800px 2px;"></div>
  <div class="anim-slide-d d1" style="font-family:Arial,sans-serif;font-size:10px;
              color:{GOLD};letter-spacing:3.5px;text-transform:uppercase;
              margin-bottom:10px;font-weight:700;">{eyebrow}</div>
  <div class="anim-fade-up d2" style="font-family:Arial,sans-serif;font-size:36px;
              font-weight:900;color:#F8FAFC;line-height:1.1;margin-bottom:8px;
              letter-spacing:-0.5px;text-shadow:0 2px 20px rgba(0,0,0,0.5);">{title}</div>
  <div class="anim-fade-up d3" style="font-family:Arial,sans-serif;font-size:13px;
              color:#94A3B8;font-weight:500;">{subtitle}</div>
</div>"""

def kpi_card(label, value, unit="", change="", change_type="neutral",
             source="", accent=GOLD, note="", delay="d1"):
    arrow = {"up": "↑", "down": "↓", "neutral": "—"}.get(change_type, "—")
    chg_color = {"up": ROSE, "down": SAGE, "neutral": "#64748B"}.get(change_type, "#64748B")
    return f"""
<div class="kpi-hover anim-fade-up {delay}" style="background:linear-gradient(145deg,#111827,#0D1421);
            border:1px solid rgba(255,255,255,0.07);border-radius:12px;
            padding:20px;position:relative;overflow:hidden;height:100%;
            animation:borderGlow 4s ease-in-out infinite;">
  <div style="position:absolute;top:0;left:0;right:0;height:3px;
              background:linear-gradient(90deg,{accent},{accent}66,transparent);"></div>
  <div style="position:absolute;bottom:-30px;right:-20px;width:80px;height:80px;
              border-radius:50%;background:radial-gradient(circle,{accent}12,transparent);
              pointer-events:none;"></div>
  ...
  <div class="anim-count" style="font-family:Arial,sans-serif;font-size:34px;
              font-weight:900;color:{accent};line-height:1;letter-spacing:-1.5px;
              text-shadow:0 0 20px {accent}44;">
      {value}<span style="font-size:17px;color:{accent};opacity:.75;font-weight:700;">{unit}</span>
  </div>
  {"<div style='font-family:Arial,sans-serif;font-size:11px;color:"+chg_color+";margin-top:9px;font-weight:700;'>"+arrow+" "+change+"</div>" if change else ""}
  {"<div style='font-family:Arial,sans-serif;font-size:9px;color:#64748B;margin-top:4px;font-weight:600;'>"+source+"</div>" if source else ""}
  {"<div style='font-family:Arial,sans-serif;font-size:9px;color:#64748B;margin-top:2px;'>"+note+"</div>" if note else ""}
</div>"""

def section_title(title, subtitle="", accent=GOLD):
    return f"""
<div class="anim-slide-r" style="margin:28px 0 16px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:5px;">
    <div style="width:4px;height:30px;background:linear-gradient(180deg,{accent},{accent}44);
                border-radius:2px;flex-shrink:0;
                box-shadow:0 0 10px {accent}55;"></div>
    <div style="font-family:Arial,sans-serif;font-size:20px;font-weight:900;
                color:#F8FAFC;letter-spacing:-0.3px;">{title}</div>
  </div>
  {"<div style='font-family:Arial,sans-serif;font-size:11px;color:#94A3B8;margin-left:16px;letter-spacing:1px;font-weight:600;text-transform:uppercase;'>"+subtitle+"</div>" if subtitle else ""}
</div>"""

def formula_box():
    return f"""
<div class="anim-fade-up d2" style="background:linear-gradient(135deg,#0D1421,#111827);
            border:1px solid rgba(212,160,23,0.3);border-left:4px solid {GOLD};
            border-radius:0 10px 10px 0;padding:20px 24px;margin:14px 0;
            font-family:Arial,sans-serif;overflow-x:auto;
            box-shadow:0 4px 24px rgba(0,0,0,0.3),0 0 0 1px rgba(212,160,23,0.1);">
  <div style="font-size:10px;color:#64748B;letter-spacing:2.5px;
              text-transform:uppercase;margin-bottom:12px;font-weight:700;">Structural OLS Model</div>
  <div style="font-size:13px;line-height:2.6;white-space:nowrap;">
    ...
    <span style="color:#CBD5E1;">α</span>
    <span style="color:#B8C4D0;font-size:10px;"> [neutral rate] </span>
    <span style="color:#8B95A3;">+ </span>
    <span style="color:{TEAL};">λ · OPR<sub>t</sub></span>
    <span style="color:#B8C4D0;font-size:10px;"> [CBSL transmission] </span>
    <span style="color:#8B95A3;">+ </span>
    <span style="color:{SAGE};">β₁ · (US10Y<sub>t</sub> + σ<sub>LKA</sub>)</span>
    <span style="color:#B8C4D0;font-size:10px;"> [Mundell-Fleming] </span>
    <span style="color:#8B95A3;">+ </span>
    <span style="color:{ROSE};">γ₁·D/GDP<sub>t</sub> + γ₂·FD<sub>t</sub></span>
    <span style="color:#B8C4D0;font-size:10px;"> [solvency/crowding-out] </span>
    <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <span style="color:#8B95A3;">+ </span>
    <span style="color:{WARM};">ϕ · (Oil<sub>t</sub> × W)</span>
    <span style="color:#B8C4D0;font-size:10px;"> [cost-push] </span>
    <span style="color:#8B95A3;">+ </span>
    <span style="color:{VIOLET};">θ · S<sub>t</sub></span>
    <span style="color:#B8C4D0;font-size:10px;"> [seasonal] </span>
    <span style="color:#8B95A3;">+ </span>
    <span style="color:{GOLD};">Ω · (1/GOR<sub>t</sub>)</span>
    <span style="color:#B8C4D0;font-size:10px;"> [ext. buffer] </span>
    <span style="color:#8B95A3;">+ </span>
    <span style="color:{TEAL};">δ·(π<sub>t</sub>−π*)</span>
    <span style="color:#B8C4D0;font-size:10px;"> [Fisher gap] </span>
    <span style="color:#8B95A3;">+ ε<sub>t</sub></span>
  </div>
</div>"""

def source_note(text):
    return f"""
<div style="background:rgba(212,160,23,0.06);border-left:2px solid rgba(212,160,23,0.35);
  ...
  <div style="font-family:Arial,sans-serif;font-size:10px;color:#94A3B8;
              line-height:1.8;font-weight:500;">{text}</div>
</div>"""

def forecast_card(tenor, value, ci_lo, ci_hi, actual, color):
    diff = value - actual
    dir_sym = "↑" if diff > 0.05 else ("↓" if diff < -0.05 else "→")
    dir_col = ROSE if diff > 0.05 else (SAGE if diff < -0.05 else "#64748B")
    return f"""
<div class="kpi-hover anim-scale" style="background:linear-gradient(145deg,#0D1421,#111827);
  ...
  <div style="position:absolute;bottom:-30px;right:-30px;width:100px;height:100px;
              border-radius:50%;background:radial-gradient(circle,{color}14,transparent);"></div>
  ...
  <div style="font-family:Arial,sans-serif;font-size:9px;color:#94A3B8;
              ...
              font-weight:700;">{tenor}</div>
  <div class="anim-count" style="font-family:Arial,sans-serif;font-size:50px;
              font-weight:900;color:{color};line-height:1;letter-spacing:-2px;
              text-shadow:0 0 30px {color}55;">{value:.2f}
    <span style="font-size:24px;opacity:.8;font-weight:700;">%</span>
  </div>
  <div style="font-family:Arial,sans-serif;font-size:10px;color:#94A3B8;
              margin-top:12px;font-weight:600;">95% CI: {ci_lo:.2f}% – {ci_hi:.2f}%</div>
  <div style="font-family:Arial,sans-serif;font-size:11px;
              margin-top:7px;color:{dir_col};font-weight:700;">
      {dir_sym} vs current {actual:.2f}%</div>
</div>"""

def adj_panel_header(title, icon, color=GOLD):
    return f"""
<div class="anim-fade-up" style="background:linear-gradient(135deg,#131B27,#0D1421);
            border:1px solid rgba(212,160,23,0.25);border-radius:12px 12px 0 0;
            padding:15px 18px;border-bottom:1px solid rgba(212,160,23,0.15);
            display:flex;align-items:center;gap:12px;">
  <span style="font-size:20px;">{icon}</span>
  <div style="font-family:Arial,sans-serif;font-size:14px;
              font-weight:800;color:#F1F5F9;letter-spacing:-0.2px;">{title}</div>
  <div style="margin-left:auto;width:10px;height:10px;border-radius:50%;
              background:{color};
              box-shadow:0 0 10px {color},0 0 20px {color}55;
              animation:pulseDot 2.2s ease-in-out infinite;"></div>
</div>"""

def adj_panel_body_start():
    return f"""
<div style="background:linear-gradient(180deg,#111827,#0D1421);
            border:1px solid rgba(212,160,23,0.2);border-top:none;
...
with st.sidebar:
    st.markdown(f"""
    <div style="background:linear-gradient(180deg,#0D1421 0%,#0A0D12 100%);
                border-bottom:1px solid rgba(212,160,23,0.25);
                padding:26px 20px 22px;
                position:relative;overflow:hidden;
                animation:slideRight .5s cubic-bezier(.22,.68,0,1.2) both;">
      <div style="position:absolute;top:-30px;right:-30px;width:120px;height:120px;
                  border-radius:50%;background:radial-gradient(circle,rgba(212,160,23,0.1),transparent);
                  pointer-events:none;"></div>
      <div style="font-family:Arial,sans-serif;font-size:22px;
                  font-weight:900;color:{GOLD};letter-spacing:-0.5px;
                  line-height:1.15;text-shadow:0 0 20px rgba(212,160,23,0.35);">LKA<br>Yield Engine</div>
      <div style="font-family:Arial,sans-serif;font-size:9px;
                  color:#64748B;letter-spacing:2.5px;text-transform:uppercase;
                  margin-top:6px;font-weight:700;">Sri Lanka · T-Bill Intelligence</div>
      <div style="font-family:Arial,sans-serif;font-size:11px;
                  color:#94A3B8;margin-top:12px;padding-top:12px;
                  border-top:1px solid rgba(255,255,255,0.07);font-weight:600;">
          📅 {TODAY_STR}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="padding:18px 20px;border-top:1px solid rgba(255,255,255,0.06);margin-top:4px;">
      <div style="font-family:Arial,sans-serif;font-size:9px;color:#64748B;
                  letter-spacing:2.5px;text-transform:uppercase;margin-bottom:14px;font-weight:700;">
        Live Yields · CBSL WEI</div>
      <div style="font-family:Arial,sans-serif;font-size:13px;line-height:2.4;font-weight:600;">
        <span style="color:{GOLD};">91D &nbsp;</span>
        <span style="color:#CBD5E1;">→ {CUR['y91']:.2f}%</span><br>
        <span style="color:{TEAL};">182D </span>
        <span style="color:#CBD5E1;">→ {CUR['y182']:.2f}%</span><br>
        <span style="color:{VIOLET};">364D </span>
        <span style="color:#CBD5E1;">→ {CUR['y364']:.2f}%</span><br>
        <span style="color:#94A3B8;">OPR &nbsp;</span>
        <span style="color:#CBD5E1;">→ {CUR['opr']:.2f}%</span><br>
        <span style="color:{SAGE};">GOR &nbsp;</span>
        <span style="color:#CBD5E1;">→ ${CUR['gor']:.3f}Bn</span>
      </div>
    </div>
    <div style="padding:14px 20px;border-top:1px solid rgba(255,255,255,0.06);">
      <div style="font-family:Arial,sans-serif;font-size:10px;color:#64748B;
                  letter-spacing:1px;line-height:2;font-weight:600;">
        CBSL WEI · 06-Mar-2026<br>CBSL MPR No.1/2026<br>World Bank · IMF<br>
        US Fed FRED (DGS10) · EIA<br><br>
        <span style="color:{ROSE};font-weight:700;">⚠ Not financial advice</span>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
if page == "Dashboard":
    ...
    if CUR["brent"] > 78:
        ...
        <div style="background:#FEF2F2;border:1px solid #FECACA;
          ...
          <span style="font-family:Arial,Helvetica,sans-serif;font-size:11px;color:#991B1B;">
    ...
    with ch2:
        ...
        <div style="background:{PANEL};border:1px solid {BORDER};border-radius:10px;
          ...
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:9px;color:#94A3B8;
                      letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;">
              Term Structure Today</div>
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:9px;color:#CBD5E1;
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:20px;">
      {"".join(f'<span style="background:rgba(212,160,23,0.08);border:1px solid rgba(212,160,23,0.18);color:{GOLD};font-family:Arial,Helvetica,sans-serif;font-size:9px;padding:3px 10px;border-radius:12px;letter-spacing:1px;text-transform:uppercase;">{t}</span>'
    ...
    with pb:
        ...
        <div style="background:#F0EDE6;border:1px solid {BORDER};border-radius:6px;
          ...
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:{SAGE};">
    ...
    with pd_:
        ...
        <div style="background:#F0EDE6;border:1px solid {BORDER};border-radius:6px;
                    padding:10px 12px;margin-top:8px;">
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:9px;color:#B8C4D0;margin-bottom:4px;">
              External buffer contribution preview</div>
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:{GOLD};">
              Ω / GOR = {omega:.2f} / {gor:.3f} = <b>{omega/gor:.3f}%</b></div>
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:9px;color:#B8C4D0;margin-top:6px;">
    ...
    with pe:
        ...
        <div style="background:#F0EDE6;border:1px solid {BORDER};border-radius:6px;
          ...
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:{WARM};">
    ...
    with pf:
        ...
        <div style="background:#F0EDE6;border:1px solid {BORDER};border-radius:6px;
          ...
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:{gap_color};">
    ...
    with pg:
        ...
        st.markdown(f"""
        <div style="font-family:Arial,Helvetica,sans-serif;font-size:10px;color:#64748B;
    ...
    with ph:
        ...
        <div style="background:#F0EDE6;border:1px solid {BORDER};border-radius:6px;
          ...
          <div style="font-family:Arial,Helvetica,sans-serif;font-size:10px;color:#94A3B8;margin-bottom:6px;">
              Current spread implied by offsets</div>
          <div style="display:flex;gap:16px;">
            <div style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:{GOLD};font-weight:600;">
                91D → {off91:+.2f}%</div>
            <div style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:{TEAL};font-weight:600;">
                182D → {off182:+.2f}%</div>
            <div style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:{VIOLET};font-weight:600;">
                364D → {off364:+.2f}%</div>
          </div>
        </div>""", unsafe_allow_html=True)
    ...
    with dc1:
          ...
          <table style="width:100%;border-collapse:collapse;">
            <thead>
              <tr style="background:#F0EDE6;border-bottom:1px solid {BORDER};">
                <th style="font-family:Arial,Helvetica,sans-serif;font-size:9px;color:#94A3B8;
            ...
            html_d += f"""
              <tr style="border-bottom:1px solid {BORDER};">
                <td style="font-family:Arial,Helvetica,sans-serif;font-size:11px;
                           color:#CBD5E1;padding:9px 14px;">{term}</td>
                ...
                <td style="padding:9px 14px;text-align:right;">
                  <div style="display:flex;align-items:center;justify-content:flex-end;gap:8px;">
                    ...
                    <span style="font-family:Arial,Helvetica,sans-serif;font-size:10px;color:#94A3B8;
        ...
        html_d += f"""
              <tr style="background:#F0EDE6;border-top:2px solid {GOLD}33;">
                <td style="font-family:Arial,Helvetica,sans-serif;font-size:11px;color:{GOLD};
                           padding:10px 14px;font-weight:700;">Base Sum (91D)</td>
                <td style="font-family:Arial,Helvetica,sans-serif;font-size:16px;color:{GOLD};
        ...
        for lbl_, val_, col_, theory_ in sens:
            ...
            <div style="display:flex;align-items:center;gap:10px;
              ...
              <div style="font-family:Arial,Helvetica,sans-serif;font-size:10px;
                          color:#94A3B8;width:130px;flex-shrink:0;">{lbl_}</div>
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div style="background:#EFF6FF;border:1px solid #BFDBFE;
      ...
      <span style="font-family:Arial,Helvetica,sans-serif;font-size:11px;color:#1E40AF;">
    ...
    <div style="background:{PANEL};border:1px solid rgba(184,134,11,0.3);
      ...
      <div>
        ...
        <div style="font-family:Arial,Helvetica,sans-serif;font-size:38px;
...
# ══════════════════════════════════════════════════════════════════════════════
        ...
        <div style="background:{PANEL};border:1px solid {BORDER};border-radius:10px;
          ...
          <div style="padding-left:18px;">
            ...
            <div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;
                        color:#94A3B8;line-height:1.8;">{content}</div>
          </div>
        </div>"""
    ...
    <div style="background:#FFF7F7;border:1px solid #FECACA;
      ...
      <div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#94A3B8;
                  line-height:2.1;">
